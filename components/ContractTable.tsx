                  ...
                  <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                    {data.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td className="px-12 py-4 text-sm font-normal text-gray-700 dark:text-gray-400 whitespace-nowrap">
                          {item.price > 0
...
export interface ContractSignHandles {
  resetForm: () => void;
  getFormData: () => { fullName: string; email: string; signature: string | null };
}
interface ContractSignProps {
  onFormDataChange?: () => void;
}

export const ContractSign = forwardRef<ContractSignHandles, ContractSignProps>(({ onFormDataChange }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    getFormData: () => {
      const signature = sigCanvas.current?.getTrimmedCanvas().toDataURL("image/png") || null;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="my-8">
        <p className="font-normal text-bases text-gray-900 mb-2">Signature:</p>
        <SignatureCanvas
          penColor="black"
        ...
        <div className="flex justify-between mt-4">
          ...
          <button
            ...
            className={`flex items-center justify-center w-1/2 px-5 py-2 text-sm tracking-wide transition-colors duration-200 ${isFormValid ? "bg-blue-500 hover:bg-blue-600" : "bg-blue-300"} text-white rounded-lg shrink-0 sm:w-auto gap-x-2`}
          >
            Submit
          </button>
        </div>
